...
import { AddCircleFilled, CubeFilled } from "@fluentui/react-icons"
import { Button, Input, InputIcon } from "keep-react"
import moment from "moment"
import { NavLink } from "react-router-dom"

import ProductCardView from "./product-list-views/ProductCardView"
import ProductTableView from "./product-list-views/ProductTableView"

import { cancelProductPetition, getProducts } from "@/common/api/product.api"
...
const ProductList = () => {
  ...
  const getList = async (pageIndex?:number) => {
    ...
    try {
      ...
      if (!products) {
        setMessageError("no hay productos")
        return
      }
      setList(products)
      setPagination({
          ...
          <div className="flex gap-4 font-bold">
            ...
            <div className="flex flex-col">
              <h1 className="text-afer-950 dark:text-afer-400 font-black text-4xl leading-none">
                {recentSearch === "" ? "Productos" : recentSearch}
              </h1>
              {
                loading ? (
        ...
        {!loading && list.length > 0 && (
          <>
            <div className="flex flex-col gap-8 justify-center pb-20 pt-4">
              {view === VIEW_TYPE.TABLE && <ProductTableView list={list} />}
              {view === VIEW_TYPE.CARD && <ProductCardView list={list} />}
